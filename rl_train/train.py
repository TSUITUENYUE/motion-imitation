    ...
    def __init__(self, runner, project_name="go2-motion-imitation", experiment_name=None, config=None):
        ...
        if self.use_wandb:
            ...
            wandb.init(
                ...
                config=config or {},
                sync_tensorboard=True,  # Auto-sync TensorBoard events
                monitor_gym=False,      # Don't automatically monitor gym envs
            )
            print(f"W&B logging initialized for project: {project_name}")
    
    def log_iteration(self, iteration, infos):
        """Log metrics to W&B at each iteration"""
        if not self.use_wandb:
            return
            
        # Extract relevant metrics from the 'infos' dict
        metrics = {}
...
def main():
    ...
    parser.add_argument("--file", type=str, default="output/run_retargeted.npy", 
    ...
    # Generate experiment name from motion file
    ...
    # Use W&B logging by default, unless explicitly disabled with --no-wandb
    ...
    # Define custom callback for runner
    def custom_callback(iteration, infos):
        # Log to W&B if enabled
        if wandb_callback:
            wandb_callback.log_iteration(iteration, infos)
    
    try:
        # Add custom callback to runner
        runner.custom_callback = custom_callback
